...
fn parse_il_transactions(value: &Value) -> Result<Vec<bytes::Bytes>, RpcErr> {
    ...
    // the felix314159/hive#3 testForkchoiceUpdatedAcceptsLargeIL test). The
    // size cap is enforced in `crates/blockchain/inclusion_list_builder.rs`
    // on the build side; this receive path imposes no upper bound.
    Ok(out)
}
